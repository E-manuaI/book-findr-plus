...
export async function searchRecentReleases(monthsBack: number = 3, startIndex: number = 0): Promise<SearchResult> {
  ...
  const results = await Promise.all(
    [`manga+${currentYear}`, `manga+${lastYear}`].map(q =>
      fetch(
...
// Each "page" rotates through a set of publisher-targeted queries using startIndex
...
export async function searchUpcoming(startIndex: number = 0): Promise<SearchResult> {
  ...
  // Rotate through publisher queries + year queries, using startIndex for pagination
  const queries = [
    ...UPCOMING_QUERIES,
    `manga ${currentYear}`,
    `manga ${nextYear}`,
