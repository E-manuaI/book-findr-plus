  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      if (res.status === 503 || res.status === 429) {
        if (i < retries) {
          await delay(600 * (i + 1)); // 600ms, then 1200ms
          continue;
        }
        return { items: [] };
      }
...
// Rotate through queries one at a time per call to stay under rate limits
...
export async function searchUpcoming(startIndex: number = 0): Promise<SearchResult> {
  ...
  // Pick 2 queries per call based on startIndex to paginate through them over time
  const pageSize = 2;
  ...
  for (const q of activeQueries) {
    ...
    for (const item of data.items || []) {
      ...
      seen.add(book.id);
      if (!book.publishedDate) continue;
      if (new Date(book.publishedDate) > now) books.push(book);
    }
    await delay(300);
  }
