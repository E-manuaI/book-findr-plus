...
const Index = () => {
  ...
  const loadUpcomingBatch = useCallback(async (startIdx: number, existing: Book[]) => {
    // Use ref to prevent overlapping calls — ref updates are synchronous unlike setState
    ...
    try {
      ...
      const merged = [...existing, ...result.books].filter(
        (b, i, arr) => arr.findIndex(x => x.id === b.id) === i
      );
      setUpcomingBooks(merged);
      setUpcomingStartIndex(startIdx + 40);
      if (result.books.length === 0 || merged.length >= 100) {
          ...
          <TabsContent value="upcoming">
            ...
            <div className="py-8 text-center">
              {upcomingLoading ? (
                ...
                <button
                  onClick={() => loadUpcomingBatch(upcomingStartIndex, upcomingBooks)}
